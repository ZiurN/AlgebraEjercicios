...
%Ejercicio 12--------------------------------------
        ...
        \item 
            \begin{enumerate}[label = \alph*)]
                ...
                \colorbox{g}{\textit{Verdadero}}, cualquier elemento que no sea mayor que 5, necesariamente es menor que 8, y para los elementos que no son menores que 8, necesariamente son mayores que 5; adem\'as, existen elementos que cumplen simultaneamente las dos condiciones.\\
                ...
                \colorbox{g}{\textit{Falso}}. Esta afirmaci\'on sugiere que los n\'umeros naturales est\'an acotados superiormente, lo cual es falso; esto se puede demostrar por el axioma del supremo.\footnote{Se deja una pequeña demostraci\'on al final de las notas.}\\
                \item $\exists n \in \naturales$, $ \forall m \in \naturales$, $m > n$\\
                \colorbox{g}{\textit{Falso}}, ya que si $m=1$, no existe ning\'un n\'umero $n$ que cumpla la condici\'on dada.\\
            \end{enumerate}
            \newpage
            ...
            \begin{enumerate}[label = \alph*)]
                ...
                \begin{center}
                    \noindent\fbox{
                        \parbox[c]{0.7\textwidth}{
                            \textit{\underline{Negaci\'on:}}\\
                            \textit{Existe al menos un elemento $n$ que pertenece a $\naturales$, tal que $n$ es estrictamente menor que 5.}
                        }
                    }
                \end{center}
                \vspace{12pt}
                ...
                \begin{center}
                    \noindent\fbox{
                        \parbox[c]{0.7\textwidth}{
                            \textit{\underline{Negaci\'on:}}\\
                            \textit{Todo elemento de $\naturales$ es estrictamente menor que 5.}
                        }
                    }
                \end{center}
                \vspace{12pt}
                \colorbox{g}{\textit{Falso}}, ya que existe no uno, sino infinitos elementos de $\naturales$ que son mayores que 5.\\
                \item $\neg(\forall n \in \naturales$, $n\geq 5 \vee n\leq 8)=\exists n \in \naturales$, $n<5 \wedge n>8$\\ 
                \begin{center}
                    \noindent\fbox{
                        \parbox[c]{0.7\textwidth}{
                            \textit{\underline{Negaci\'on:}}\\
                            \textit{Existe al menos un elemento de $\naturales$ que es menor que 5 y mayor que 8.}
                        }
                    }
                \end{center}
                \vspace{12pt}
                \colorbox{g}{\textit{Falso}}, pues no se puede dar simultaneamente que alg\'un n\'umero sea menor que 5 y mayor que 8.\\
                \item $\neg(\exists n \in \naturales$, $n\geq 5 \wedge n\leq 8)=\forall n \in \naturales$, $n<5 \vee n>8$\\ 
                \begin{center}
                    \noindent\fbox{
                        \parbox[c]{0.7\textwidth}{
                            \textit{\underline{Negaci\'on:}}\\
                            \textit{Todo n\'umero natural es, o bien estrictamente menor que 5, o bien estrictamente mayor que 8.}
                        }
                    }
                \end{center}
                \vspace{12pt}
                ...
                \begin{center}
                    \noindent\fbox{
                        \parbox[c]{0.7\textwidth}{
                            \textit{\underline{Negaci\'on:}}\\
                            \textit{Existe un n\'umero n que pertenece a los naturales, tal que para todos los dem\'as n\'umeros m, m es mayor o igual a n.}
                        }
                    }
                \end{center}
                \vspace{12pt}
                ...
                \begin{center}
                    \noindent\fbox{
                        \parbox[c]{0.7\textwidth}{
                            \textit{\underline{Negaci\'on:}}\\
                            \textit{Para todo n\'umero natural n, existe un n\'umero m, tamb\'ien natural, tal que m es menor o igual a n.}
                        }
                    }
                \end{center}
                \vspace{12pt}
                \colorbox{g}{\textit{Verdadero}}, ya que si tomamos $m=1$, todo n\'umero $n$ natural es mayor o igual a 1.\\
            \end{enumerate}
            \newpage
            \item En cada uno de los casos siguentes, decidir si las dos proposiciones tienen el mismo valor de verdad. Dar un contraejemplo cuando no es el caso.\\
            \begin{enumerate}[label = \alph*)]
                \item $\exists x$, $\forall y$, $p(x,y)$ y $\forall y$, $\exists x$, $p(x,y)$\footnote{El ejemplo usado para este punto es literalmente el mismo que se toma en \textit{Proofs and Fundamentals A First Course in Abstract Mathematics Second Edition}}\\\\
                Sea $p(x,y)=x+y^2=3$ y supongamos que en el conjunto de pertenencia tiene sentido $p(x,y)$, y que tanto $x$ como $y$ pertenecen a dicho conjunto (Por ejemplo suponiendo que el conjunto es $\real$). La primera proposici\'on dice: ``\textit{Existe un n\'umero $x$, tal que para todo n\'umero $y$, $x+y^2=3$}'', lo cual es falso, es decir, no existe un valor de $x$ tal que la igualdad se cumpla para todos y cada uno de los diferentes valores de $y$ con los que se haga una prueba.\\\\
                Tomando ahora la segunda proposici\'on tenemos: ``\textit{Para todo n\'umero $y$, existe un n\'umero $x$, tal que $x+y^2=3$}'', y esto si se puede dar, ya que para cada valor de $y$ siempre se puede encontrar un valor de $x$, despejando dicho valor en la ecuaci\'on.\\
                \item $\forall x$, $p(x)$ y $\nexists x$, $\neg p(x)$\\\\
                Cuando se dice que todo elemento de un conjunto determinado cumple una condici\'on o tiene una propiedad determinada, es lo mismo que decir que no existe ning\'un elemento que no la cumpla, que es justamente lo que se expresa en la segunda proposiciº'on, por lo que estas dos proposiciones deben tener el mismo valor de verdad.
            \end{enumerate}
        \end{enumerate}
%Ejercicio 13 -------------------------------------
        \item Determinar cu\'ales de las siguentes afirmaciones son verdaderas cualesquiera sean los subconjuntos $A$, $B$ y $C$ de un conjunto referencial $V$ y cu\'ales no. Para las que sean verdaderas, dar una demostraci\'on, para las otras dar un contraejemplo.\\
        
%Ejercicio 13 -------------------------------------
        \item $A$, $B$ y $C$ subconjuntos de un conjunto referencial $V$. Probar que
    \end{enumerate}
\end{document}
